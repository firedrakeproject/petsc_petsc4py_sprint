...
def apidoc_class(cls, outer=None, done=None):
    skip = {
        '__doc__',
        '__dict__',
        '__module__',
    ...
    for name in members(keys):
        ...
        if is_class(attr):
            ...
            lines.add = apidoc_class(attr, outer=cls_name)
            continue
    ...
    for name in members(keys):
        ...
        if is_constant(attr):
            ...
            lines.add = apidoc_data((name, attr))
            continue
...
def apidoc_petsc4py_PETSc(done=None):
...
def generate(filename):
    ...
    with open(filename, 'w') as f:
        for line in apidoc_petsc4py_PETSc():
            print(line, file=f)
